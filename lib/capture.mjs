...
export async function capture(config, scenes) {
  ...
  const directorJs = fs.readFileSync(path.join(__dirname, 'inject/director.js'), 'utf8');
  ...
  await page.addInitScript({ content: [config.initScript || '', cursorJs, directorJs].join('\n;\n') });
...
async function captureScene(page, scene, config, ffmpeg) {
  ...
  const url = scene.route.startsWith('http') ? scene.route : config.baseUrl.replace(/\/$/, '') + '/' + scene.route;
