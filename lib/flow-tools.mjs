...
export async function validateFlow({ baseUrl, flowFile }) {
  ...
  try {
    for (const flow of manifest.flows) for (const scene of flow.scenes) {
      ...
      if (scene.waitFor && !errors.length) {
        const ready = await page.evaluate(async ({ selector, timeoutMs }) => !!(await window.ScreenReelCore.waitFor(document, selector, 'visible', timeoutMs)), { selector: scene.waitFor, timeoutMs: Number(scene.timeoutMs) || 8000 }).catch(() => false);
        if (!ready) errors.push(`waitFor: ${scene.waitFor} did not become visible`);
      }
      if (scene.settleMs && !errors.length) await page.waitForTimeout(Number(scene.settleMs));
      const actions = [];
